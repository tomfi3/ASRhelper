...
Usage:
    python main.py [--year YEAR] [--sites SITE1,SITE2,...] [--borough BOROUGH]
"""
...
def fetch_automatic_analyser_data(site_code, year):
    """Fetch hourly NO2 data from automatic analyser and compute monthly/annual stats."""
    ...
    try:
        ...
        if not hourly_records:
            logger.warning(f"No hourly NO2 data returned for {site_code}")
            return {}, {}
        ...
        logger.error(f"Failed to fetch automatic data for {site_code}: {e}")
        return {}, {}
...
def process_site(site_code, year, output_dir):
    ...
    # If no annual_stats from hourly data, create empty
    if not annual_stats:
        annual_stats = {
            "annual_mean": None,
            "data_capture": 0,
...
def main():
    ...
    logger.info(f"ASR Helper - Bias Adjustment Questionnaire Generator")
    ...
    logger.info(f"2. Fill in the tube supplier/laboratory and preparation method")
    logger.info(f"3. Add diffusion tube exposure start/end dates if available")
    logger.info(f"4. Apply national bias adjustment factor if local factor unavailable")
    logger.info(f"   Download from: https://laqm.defra.gov.uk/air-quality/air-quality-assessment/national-bias/")
